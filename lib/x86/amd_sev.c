...
static inline int snp_set_vmsa(void *va, bool vmsa)
{
	 ...
	 * using RMPADJUST instr. However, for instruction to succeed,
	 * we must target permissions of a lesser previliged VMPL level.
...
void bringup_snp_aps(void)
{
	 ...
	 * For now, make a check for alignment and fail if page is not
	 * 4K aligned.
	 */
	vmsa = (struct sev_es_save_area *)alloc_page();

	if (!vmsa) {
		printf("VMSA page allocation failure.\n");
		return;
	}
